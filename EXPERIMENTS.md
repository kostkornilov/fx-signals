...
В основной конфигурации указаны группы A+B. В матричном эксперименте проверены LogReg на A, A+B, A+B+C и A+B+C+D, CatBoost на A+B и A+B+C+D. Обозначение в колонке `feature_groups` накопительное: например, `A,B,C` означает, что модель одновременно получила объяснимые индикаторы, числовую силу этих индикаторов и техническую динамику курса. Пустая `feature_groups` у momentum, level, reversal, seasonality и random означает, что это не обучаемые модели, а отдельные правила или случайный baseline.
...
Сравнение групп признаков не показывает, что больше данных автоматически даёт лучший результат. На финальном OOT LogReg только на группе A получила средний lift 1,034 при 97 сигналах, а после добавления A+B+C — 0,968 при 106 сигналах. Конфигурации LogReg A+B и A+B+C+D на OOT не сформировали ни одного сигнала при выбранных порогах, поэтому их нельзя считать ни успешными, ни корректно сравнимыми по lift. CatBoost A+B дал средний OOT lift 1,321 при 105 сигналах. CatBoost A+B+C+D показал средний lift 1,175 и высокую среднюю выгоду около 180 б.п., но сформировал всего 16 сигналов по всем пяти коридорам; этот результат слишком разрежен и чувствителен к единичным наблюдениям. Таким образом, в текущем эксперименте наиболее полезной оказалась компактная объяснимая комбинация A+B, а добавочная ценность технических и внешних признаков C+D не подтверждена устойчиво.
