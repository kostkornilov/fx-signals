...
from fx_signal.train import (
    _random_signal, _walk_forward_model, build_frame, evaluate_all_splits,
)
...
names = (('reconstructed_old_metrics.csv', 'predictions.csv', 'folds.json') if not suffix
         else (f'old_schema_metrics{suffix}.csv', f'predictions{suffix}.csv', f'folds{suffix}.json'))
...
for method, groups in jobs:
    ...
    m = evaluate_all_splits(work, method=method, signal_col='eval_signal',
                           target_col='target_stay_not_worse_h5', horizon=5, folds=folds)
    m['feature_groups'] = ','.join(groups)
    metrics.append(m)
    for fold in folds:
...
pd.concat(metrics).to_csv(out / names[0], index=False)
pd.concat(predictions).to_csv(out / names[1], index=False)
(out / names[2]).write_text(json.dumps(audits, indent=2))
